...
这些限制来自数据量和步数，不是「模型坏了」。换 greedy 无法证明模型差；换采样也无法证明已经很好——只能证明 **日志低估了它**。
...
> 单卡 bf16 预训练约 70M Decoder-only GPT（莎士比亚全集，2000 step，val PPL 77）。训练日志的 greedy 样例会复读；同一 checkpoint 用 temperature 0.8 / top-k 50 可生成带人物名、场次标题和舞台指示的剧本片段。

**不要写：** 已能创作莎士比亚、生成质量接近 nanoGPT 教程数字、greedy 样例代表模型上限。

展示样例请用本目录这三条（尤其 sample 1），不要用 Day 6/7 日志里的 greedy 空行。若再训 `shakespeare_best`（更大 batch、更多步），新旧样例都用同一套采样超参，才和这份对照公平。
